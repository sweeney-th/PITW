...
# target comes first, then the desired replacement
...
# notice the first two, "or more", operators keep going as long as they can.
...
# all occurrences, and the "i" modifier causes the match to be case in-sensitive
...
### find all the occurrences of a string with the global modifier
...
# escape the "*" characters, and then find any number or alpha numerics
# in between them
...
# first order of business: avoid the "x"s and get the www. we can simply use
...
# with the ? to get all the individual occurrences of the string. that's the

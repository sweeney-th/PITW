...
# 2) they are inherently UNORDERED
# thus you can't count on them being printed in
# the same order form one execution to the next
# this seems weird at first, but you're using it
# to look them up by a key, not a value, so
# it does't matter
...
# something is in the hash and proceede accordingly as calling for
# something that ins't there will crash you out. we can avoid this
